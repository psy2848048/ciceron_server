...
def json_from_V_REQUESTS(conn, rs, purpose="newsfeed"):
    ...
    for row in rs:
        ...
        item = dict(
                ...
                request_registeredTime=int(row[24].strftime("%s")),
                request_dueTime=int(row[27].strftime("%s")),
                request_transStartTime=int(row[55].strftime("%s")),
                request_expectedTime=int(row[25].strftime("%s")),
