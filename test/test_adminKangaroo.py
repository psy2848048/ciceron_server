...
class KangarooAdminTestCase(TestCase):
	...
	def test_getAdminKangarooTagProvideImageBinary(self):
		# GET
		res = self.app.get('/api/v2/admin/kangaroo/tag/12/img/3/img.jpg')
		self.assertEqual(res.status_code, 200)


	def test_putAdminKangarooTagProvideImageBinary(self):
		# PUT

		with open('./testdata/test_image.jpg') as test:
			imgBytesIO = io.BytesIO(test.read())
		res = self.app.put('/api/v2/admin/kangaroo/tag/12/img/3/abc.jpg',
		                   content_type='multipart/form-data',
		                   data=dict(
			                   tag_id=12,
			                   img_id=3,
			                   photo_bin=imgBytesIO
		                   )
		                   )


	def test_deleteAdminKangarooTagProvideImageBinary(self):
		res = self.app.delete('/api/v2/admin/kangaroo/tag/12/img/3/abc.jpg')
		self.assertEqual(res.status_code, 200)
